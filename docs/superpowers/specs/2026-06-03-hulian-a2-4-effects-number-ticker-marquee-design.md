...
| **Marquee** | Tailwind config keyframes + 复制子项 + `group-hover:[animation-play-state:paused]` | **纯 CSS keyframe**（关键帧落 `@hulianui/tokens` 的 preset.css）+ 瑚琏 token + `motion-reduce:` 变体 | **无运行时 / 无 `"use client"`**（纯 CSS/RSC，同 Breadcrumb/Alert） |
...
export function Marquee({
  ...
  return (
    <div
      ...
      style={{
        gap,
        // 喂给 keyframe 的 CSS 变量（关键帧在 @hulianui/tokens 的 preset.css，见 §3.3）
...
### 3.3 关键帧落 `@hulianui/tokens/preset.css`（CSS 侧动效的 SSOT）
...
- **为何不放组件内**：避免 `<style>` 散写关键帧、避免跨包 CSS import 的 Next 处理风险。preset.css 是已验证被处理+下发的 CSS SSOT。代价：Marquee 的关键帧依赖 `@hulianui/tokens`（一行 `@keyframes`），实现阶段 git add 须含 `packages/tokens/src/preset.css`，文档化此跨包耦合。
...
| 颜色 | `text-foreground`（语义 token） | 中性（子项自带，容器不染色） | `@hulianui/tokens` 语义层 |
...
| **Step 2 — Marquee** | preset.css 加 `@hulianui/tokens` keyframe + 四件套 TDD（DOM 结构/复制份/类/aria/CSS 变量）+ 主 barrel export | typecheck + marquee 单测绿；无缝/hover/明暗截图 |
