...
- ~~audit 输出必须区分「有场景没采用」与「没场景」~~ → **已实现**，判据就是本文建议的邻近信号。
- ~~考虑用「高层业务组件采用度」替代裸覆盖率作为 audit 主指标~~ → **已采纳**为主指标。
- ~~存量项目体检还需版本感知与增量基线~~ → 增量基线已实现；**版本感知已否决**（见下）。

## 后续：本文结论的落地情况（2026-08-01）

`audit_hulian_adoption` 已实现（#43），本文的三条建议全部进了实现，另有两处修正：

1. **「发现四：版本分布支持版本感知」这条已被否决。** 本库是个人项目、迭代快，目标态是所有
   消费项目跟最新；为「给旧版项目推荐它装得上的组件」维护一套版本化 registry，是拿持续的
   维护成本去支撑一个我们本来就不想要的状态。改为如实报出落后幅度并建议升级，建议内容一律
   按最新版给。发现四的数据仍然成立，只是结论从「做版本感知」变成「让落后的项目升上来」。
2. **发现五的聚类被实现验证了 10/11。** 唯一分歧是 `ins-admin`：本文归 `admin-console`，
   自动判定给 `config-tool`（它用的是 `secret-field` / `json-viewer` / `log-viewer` / `snippet`，
   壳是 admin 而内容是配置）。这类横跨形态的项目会把次名候选一并报出，由人覆盖 ——
   实现不假装能替产品做这个判断。

另外实现阶段暴露了一条本文没预见的坑：`marketing` modifier 会因为项目在登录页用了一个文字
特效而成立，进而把 `border-beam` 之类推进运维后台 —— 正是发现三警告的「往中后台塞特效」。
修法是给 surface 加机器可判定的 `avoidCategories`：**surface 决定组件语言，modifier 的建议
不得越过它的边界**。
