...
- 1412273: 文档类 tool 按**消费方实装**的 `@hulianui/ui` 版本核对（#337）。
...
- d311ad3: 改正 `get_setup_guide` 与 `inspect_project` 里对「漏配 `@source`」的症状描述（#336）。
...
- 3d0dde9: `source.artifactDigests`：每条响应带上**这一次调用真正读到的**产物的 sha256（#332）
