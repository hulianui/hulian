...
> Tiles a flat dataset into a rectangle with area proportional to value, so the biggest contributors read at a glance. · data-display/collection
...
When adjacent items must be compared precisely, or the list is meant to be read in order, use [BarChart](../chart/chart.md) with `horizontal`, because area differences between neighboring cells cannot be judged accurately. For three to five items where "these add up to 100%" is the message, use [PieChart](../chart/chart.md).
