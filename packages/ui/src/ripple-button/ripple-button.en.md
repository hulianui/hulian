...
- **`variant` is about appearance, `render` is about semantics.** The note above ("use `Button variant="link"`") is about how the control *looks* — whether it has a button box. If you need something that looks exactly like this solid button but has to be an `<a>` (middle-click to open in a new tab, copy link address, visible to crawlers), that is `render={<a href="…" />}`; the ripple and the colors are unchanged (#256).
