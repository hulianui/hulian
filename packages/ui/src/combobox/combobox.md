...
| 插槽 | 类型 | 说明 |
|------|------|------|
| children | `ReactNode ｜ (value: ComboboxItemData ｜ null) => ReactNode` | 自定义触发钮内容，替换掉默认那块「已选 label ?? placeholder」。传函数即按已选值分叉。不传 = 原行为 |
...
- **创建项的文案单点可覆盖**：`createLabel` 不传就用全局 locale 的 `combobox.create`（「使用 “xxx”」）。同一个应用里两个 `creatable` 说的常常不是同一件事——一个是「新建一个机构名」，另一个可能要写成「使用「xxx」（原文非标状态）」，括号里那句是给操作者的判断依据。此前只有全局 locale 一个出口，为一行文案得在业务里嵌一层 `ConfigProvider`。
- **接管的那个初值按范式取，不用自己传**：配 `ComboboxInput`（内联）取已选项的 label，因为输入框自己就是字段；配 `ComboboxTrigger`（弹层内搜索）取空串，因为被预填的是弹层里的**搜索框**——否则首次打开时搜索框里已经躺着已选项的全名，用户打的字直接追加在后面（#258）。范式是扫 `children` 里有没有 `ComboboxTrigger` 认出来的，所以 Trigger 被你自己的包装组件裹起来时认不出，那种情况显式传 `defaultInputValue=""`。

### ComboboxTrigger 的 children：窄格子里触发钮退化成状态图标

表格单元格里常常已经有一个字段在显示这个名字了，触发钮再显示一遍，同一个值在一个格子里出现两遍，看起来像两个字段——那种位置只放得下一枚状态图标：

```tsx
<Combobox items={companies} value={bound} onValueChange={setBound}>
  <ComboboxTrigger aria-label="绑定公司" showChevron={false} className="h-7 w-7 justify-center px-0">
    {(value) => (value ? <Link2 className="size-4 text-primary" /> : <Link2Off className="size-4 text-muted-foreground" />)}
  </ComboboxTrigger>
  <ComboboxContent searchPlaceholder="搜索公司">
    {(item) => (
      <ComboboxItem key={item.value} value={item}>
        {item.label}
      </ComboboxItem>
...
`placeholder` 顶不了这件事：它只接字符串，而且只在**未选中**时出现——一选中就掉回显示名字。把 `label` 造成图标节点也不行，那会让弹层列表跟着变成一排图标。

传节点即固定内容，传函数即按已选值分叉。⚠️ 自定义内容不会自动截断：默认那块自带 `truncate`，换掉之后长文本的省略号得自己写。
