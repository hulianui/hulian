...
- **Mark the tree whenever you can, and mark an element that actually takes up space.** A `structural` path is inferred from the DOM shape, so one extra sibling or a flipped conditional can point it somewhere else. Put `data-hulian-path` on component roots to get paths that survive re-layout. Do not put the marker on a `display: contents` wrapper: such elements generate no box, `getBoundingClientRect()` is always zero, and the zero-area check treats them as invisible, so no box is ever drawn.
...
- **Interception and clearing only happen inside the target.** When `target` is a plain container the listeners still sit on the host document (they have to, to cover the iframe case), but the component first checks whether the event landed inside `target`: clicks outside are neither `preventDefault`ed nor `stopPropagation`ed, and they do not fire `onClear`. Host navigation and inspector panels keep working, and `onClear` fires only when you click empty space inside the preview.
