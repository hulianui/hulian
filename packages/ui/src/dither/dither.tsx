...
// 8×8 Bayer 有序抖动阈值（程序式生成）
// 注意：GLSL ES 1.00 不支持数组构造器（const float m[64] = float[64](...) 是 ES 3.00 语法，
// ANGLE 下直接编译失败 → 全黑）。改用经典 bayer2 三轮递归：bayer2 给出 2×2 基矩阵
// [[0,2],[3,1]]/4，每递归一层把上层结果缩 1/4 再叠加细分层，三层即得 8×8 Bayer，
// 值域同查表版 {0..63}/64。纯 floor/fract 浮点运算，无数组、无位运算，ES 1.00 安全。
float bayer2(vec2 a) {
  a = floor(a);
...
float bayer(vec2 px) {
  return bayer4(0.5 * px) * 0.25 + bayer2(px);
}
