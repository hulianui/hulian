...
- 骨架行带 `data-loading-row` 且**整行 `aria-hidden`**，数据行两者都没有。写 E2E / 自动化时判活探针要写成 `tbody tr:not([data-loading-row])`——直接数 `tbody tr` 会把骨架行算进去，而骨架行数常常正好等于 `pageSize`，于是「等到 N 行」会在数据到达之前就被满足（#349）。
...
- **外壳按真实滚动态打 `data-overflow-left` / `data-overflow-right`**（对应侧还有被裁掉的内容才有，滚到头就摘掉），与 `scrollbar` 取值无关。冻结列的分隔阴影读的就是它：只在那一侧确实还有内容时才出现，滚到底了阴影就收掉。要做自己的溢出提示直接选 `[data-overflow-right]` 即可，不必再量 `scrollWidth`。`stickyHeader="scrollParent"` 下外壳不是滚动容器，量不到也就不打，冻结列退回常驻阴影。
