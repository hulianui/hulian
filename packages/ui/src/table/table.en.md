...
| scrollbar | `"auto" \| "always"` | `"auto"` | How the shell draws its horizontal scrollbar. `"auto"` leaves it to the browser: macOS overlay scrollbars are invisible until you scroll, so nothing tells the user there are more columns to the right. `"always"` applies the same classic scrollbar skin the `stickyScrollbar` proxy uses to the shell itself, so the bar stays visible whenever the content overflows; when nothing overflows the browser simply draws none and no height is reserved. Orthogonal to `stickyScrollbar` (one styles the shell's real scrollbar, the other adds a proxy once the table's bottom edge is below the fold); wide, tall tables are best with both on. Has no effect under `stickyHeader="scrollParent"`, where the shell does not scroll horizontally. |
| loading | `boolean` | `false` | Loading. It takes over the table body **only while there is not a single row**: it renders skeleton rows and suppresses the empty state, because on the first load the data has not arrived yet and "No data" states something nobody knows to be true. Once rows exist (paging, refresh, changed filters) this prop only sets `aria-busy` and the rows stay on screen; the translucent "updating" overlay is the outer layer's job (`ProTable` ships one). |
| loadingRows | `number` | `5` | Skeleton row count, used only in the "`loading` with no rows" case and clamped to 1..20. Pass the page size so the placeholder height matches the real table and nothing jumps once the data lands. |
...
- `loading` **draws no overlay**. It only takes over the table body in the "not a single row" case, where it renders skeleton rows; once rows exist it merely sets `aria-busy`. The translucent "updating" overlay comes from `ProTable` (which ships one) or from a wrapper of your own. The converse matters too: never let the empty state show through your own overlay, which is exactly the problem `loading` exists to solve (#349).
- Skeleton rows carry `data-loading-row` and the whole row is `aria-hidden`; data rows have neither. Write liveness probes in end-to-end tests as `tbody tr:not([data-loading-row])`: counting `tbody tr` directly includes the skeleton rows, and the skeleton row count is often exactly `pageSize`, so "wait for N rows" is satisfied before the data ever arrives (#349).
...
- **The shell reports its real scroll state as `data-overflow-left` / `data-overflow-right`** (present only while content is clipped on that side, removed once you reach the edge), regardless of the `scrollbar` value. The pinned-column divider shadows read exactly that: they show only while there is more content on that side and disappear at the edge. For a custom overflow hint, select `[data-overflow-right]` instead of measuring `scrollWidth` yourself. Under `stickyHeader="scrollParent"` the shell is not the scroll container, nothing is measured, and pinned columns fall back to a permanent shadow.
