...
| `figureAlt` | `(index: number) => string` | - | Alt text for figures split out of the stem, e.g. (1) → "Figure 1". **Used only when the reference carries no alt of its own**: a reference such as `![7x+5<5x+1](...)` (an inline formula image from an import pipeline) takes the text in the brackets. The card has no hooks and cannot read the Locale, so the default is Chinese; the QuestionEditor preview passes its own locale's `figureAlt`, and standalone English usage should pass one. |
