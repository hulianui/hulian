...
| `figureAlt` | `(index: number) => string` | - | 题干附图的 alt 文案，如 (1) → "题目附图 1"。**只在引用自己没写 alt 时用**：`![7x+5<5x+1](…)` 这种带 alt 的引用（导入线切出的公式图）直接用方括号里那段。本件无 hook 读不到 Locale，缺省中文；QuestionEditor 预览会传自己 locale 的 `figureAlt`，单独用且要英文时自己传 |
