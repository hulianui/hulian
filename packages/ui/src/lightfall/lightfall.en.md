...
Use it for an atmospheric dark hero with multicolor falling beams, a central glow, and light trails. Choose [LightRays](../light-rays/light-rays.md) for radial beams, [LightPillar](../light-pillar/light-pillar.md) for one volumetric column, or [DotPattern](../dot-pattern/dot-pattern.md) for a static lattice.
...
| density | `number` | `0.6` | Angular beam density; higher values create more closely packed rings |
...
```
```tsx
// Slow ambient backdrop with long trails and no interaction
...
- OGL/WebGL renders on the client; SSR, reduced motion, and unavailable WebGL use the static `fallback`.
