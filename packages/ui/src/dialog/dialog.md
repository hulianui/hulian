...
| `DialogContent.backdropClassName` | `string` | - | 追加到遮罩（默认 `bg-black/40 backdrop-blur-sm`），走 twMerge，可调浓度/模糊。`draggable` 的「拖过就让开」也写在这里，想按住不放就覆盖同名变体（`data-[dragged]:bg-black/40 data-[dragged]:backdrop-blur-sm`） |
...
| `DialogContent.draggable` | `boolean` | `false` | 允许按住标题行拖动对话框。把手是标题行（`title` 与 `extra` 所在那一行），行里的按钮照常点击不起拖；自画 header 时给自家元素加 `data-drag-handle`。整块不出视口，每次打开回到初始位置；**挪开之后遮罩跟着让开**（浓度降到 10% 且不再模糊），否则看不见底下的内容。见下方「可拖动」 |
...
- **挪开之后遮罩跟着让开**：默认遮罩是 40% 黑 + 模糊，表达的正是「别看后面」，与 `draggable` 的「挪开看后面」互相抵消 —— 不让开的话拖动只是把一块看不清的东西换个地方放。所以**真的产生了位移**的那一刻（不是按下，也不是开着就变），遮罩会被标上 `data-dragged`，浓度降到 10%、模糊撤掉，并且松手后不撤回。没拖过的可拖对话框层次感一点不变。

  浓度留 10% 而不是全透明是有意的：遮罩仍然吃掉整屏点击（模态就是这个语义），全透会让「看得见却点不到」变成没有任何提示的怪事。要按住原样就用 `backdropClassName` 覆盖同名变体：
