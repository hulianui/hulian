...
- **`theme="light"` currently only holds on a light page.** The token layer defines an override block for `[data-theme="dark"]` but no counterpart for `[data-theme="light"]`, so a local light island inside a dark page does not stick, while `theme="dark"` inside a light page does. Omitting `theme` and following the global theme is the recommended usage.
