...
group: advanced
tags: []
exports: [RegionSelect, toImagePoint, normalizeBox, applyAspect, boxMinSide, strokeWidthFor]
status: enriched
---
...
| onChange | `(box: RegionBox) => void` | — | 拖完一次给一个**规范化**的框（反向拖也成立） |
| onDrafting | `(box: RegionBox \| null) => void` | — | 拖拽过程中的实时框，结束回 `null` |
| minSide | `number` | `8` | 误点阈值：框短边小于它（原图像素）不触发 onChange |
...
`toImagePoint(clientX, clientY, rect, w, h)` 指针→图像素（含钳位）· `normalizeBox(a, b)` 两点→规范化框 · `applyAspect(anchor, point, aspect, w, h)` 按比例修正 · `boxMinSide(box)` · `strokeWidthFor(w)`。自己画框但不想重写这套算术时直接用。
