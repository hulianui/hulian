...
The component emits the event and nothing else: navigation, drawers, and query parameters stay in application code. Pie charts take the same prop with a `{ datum, index }` payload, because a slice *is* a data point, so there is no series to report.
