...
export interface MathTextProps {
  ...
  scriptScale?: number;
  /**
   * 认 `$…$` / `$$…$$` / `\(…\)` / `\[…\]` 分隔符：**只有分隔符内的内容按数学解析**，
   * 外面一律按纯文本原样输出（`$` 外的 `{a_n}` 就是三个字面字符，不再被猜成下标）。
   *
   * 默认 `false` = 存量行为（全串扫裸记号），因为开了它之后 `售价 $100 起，成本 $80`
   * 这类文本里的两个 `$` 会被当成一对分隔符。**你的正文里有货币金额就别开**。
   *
   * 上游数据带 `$` 时应当开它，而不是在入库处把 `$` 剥掉 —— 剥 `$` 会丢掉公式边界，
   * 那是渲染层反向污染数据。整串没有成对分隔符时自动回退到存量行为，便于半迁移的题库平滑过渡。
   *
   * 只影响「哪一段按数学解析」，不影响排版：`$$` 在本组件里仍是行内渲染。
   * 要块级居中、要 `\begin{cases}` 真排版，用 `@hulianui/ui/math` 的 Formula。
   * @default false
   */
  delimiters?: boolean;
  className?: string;
