...
| hiddenFields | `readonly ("difficulty" \| "score" \| "estimatedMinutes")[]` | - | Which fields of the metrics row are not rendered. Products where a score belongs to the paper-question pair rather than the question use it to drop the score input; the editor never writes a hidden field (switching type no longer recomputes `score`) and its value passes through `Question` untouched. With all three hidden the row disappears |
...
- **When score or timing is not part of your question model, hide it with `hiddenFields` instead of zeroing the default score** (#358). A `defaultScoreByType` full of zeros only stops the editor from inventing a score; the input is still there and teachers still fill it in - and what they type either contradicts the product ("this question is worth 3 points" inside a question bank) or gets dropped silently on submit. Once hidden, the editor writes nothing to that field: switching type no longer recomputes `score` either, because writing a field the user cannot see is a silent data change. The value itself still passes through untouched, and `validateQuestion` is unchanged - an issue on a hidden field simply has nowhere to show. Only the three fields of the metrics row can be hidden; stem, options, answer and explanation cannot.
