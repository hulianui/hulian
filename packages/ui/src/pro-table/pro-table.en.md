...
| loading | `boolean` | - | Display-mode loading state (managed mode drives its own via `request` and ignores this). It spins the refresh icon and splits into two cases: with rows on screen it lays a translucent overlay over them (the previous batch stays readable), and **with no rows at all** the inner `Table` renders skeleton rows and no overlay — an overlay with "No data" showing through says "loading" and "nothing here" at the same time. |
| loadingRows | `number` | Page size | Skeleton row count for the first load, forwarded to `Table`. Defaults to the current `pagination.pageSize`, falling back to the managed page size when no pagination is rendered; pass a value to override it. |
