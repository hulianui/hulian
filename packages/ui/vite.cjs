 ...
 * ## 为什么实现写成 CJS
 *
 * 与 `vitest-preset.cjs` 同一个理由：Vite 按消费方 `package.json` 的 `type` 决定
 * `vite.config.ts` 用 ESM 还是 CJS 加载，没有 `"type": "module"` 就走 `require`，
 * 而 `require` 加载不了 ESM —— 配置加载阶段直接失败，插件根本没机会运行（#143 报的是
 * vitest-preset，但这份插件的加载路径完全同构，一并改掉，不留同款地雷）。
 * `vite.js` 是它的 ESM 包装，两侧拿到同一个函数。
 */
...
function detectLinked() {
  try {
    // __filename 而不是 import.meta.url：本文件是 CJS（原因见文件头「为什么实现写成 CJS」）。
    // 两者取到的都是「本文件的真实路径」，判据不受影响 —— vite.js 那层 ESM 包装不参与探测。
...
module.exports = hulian;
module.exports.hulian = hulian;
module.exports.default = hulian;
