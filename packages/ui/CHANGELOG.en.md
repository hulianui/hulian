...
- `QuestionCard` gains `figureAlt?: (index) => string`: the alt text for figures split out of the stem now comes from the locale-owning parent (the QuestionEditor preview passes its own `figureAlt`). Previously it always fell back to the Chinese "题目附图 N" regardless of language, which left Chinese text on the English docs examples. <!-- parity-id: question-card-figure-alt -->
