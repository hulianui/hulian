...
import { projectStatusTone, yuan } from "../_data/status";
...
export function ProjectDetail({ id }: { id: string }) {
  ...
  if (!project) {
    return (
      <Result
        status="error"
        title="项目不存在"
        subTitle="该项目可能已被删除（demo 内存态，刷新还原）。"
      >
        <Button render={<Link href={`${ROOT}/tracking`} />}>返回项目列表</Button>
      </Result>
    );
  }
  ...
  const steps: StepsItem[] = milestones.map((m) => ({
    title: m.label,
    description: m.doneAt ? `完成 ${m.doneAt}` : `计划 ${m.plannedAt}`,
  }));
      ...
      <Card variant="outline">
        <CardBody className="p-5">
          <Descriptions
            ...
            bordered
            items={[
              { label: "发包甲方", children: project.client },
              { label: "承接班组", children: project.crew },
              { label: "项目负责人", children: project.owner },
              { label: "合同额", children: <span className="tabular-nums">{yuan(project.contractAmount)}</span> },
              { label: "开工日期", children: project.startAt },
              { label: "计划竣工", children: project.dueAt },
              { label: "项目地址", children: project.address, span: 2 },
              { label: "当前进度", children: `${project.progress}%` },
            ]}
          />
        </CardBody>
      </Card>
      ...
      <div className="grid grid-cols-1 gap-4 lg:grid-cols-3">
        ...
        <Card variant="outline">
          ...
          <CardBody className="flex flex-col gap-2 pt-1">
            <RelLink href={`${ROOT}/quotes`} icon={<FileText className="size-4" />} label="报价单" count={relQuotes.length} />
            <RelLink href={`${ROOT}/invoices`} icon={<Receipt className="size-4" />} label="发票" count={relInvoices.length} />
            <RelLink href={`${ROOT}/photos`} icon={<Images className="size-4" />} label="工作照片" count={relPhotos.length} />
          </CardBody>
        </Card>
      </div>
    </div>
