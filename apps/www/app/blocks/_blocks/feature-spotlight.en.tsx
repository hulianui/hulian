...
const CARDS: SpotlightCard[] = [
    {
        ...
        description: "Requests start on the nearest edge node and respond in milliseconds. Capacity scales with traffic, returns to zero afterward, and costs nothing while idle.",
    ...
    },
    {
        ...
        description: "Issue and renew HTTPS certificates automatically, manage encrypted secrets, and record every change in the audit trail. We handle compliance so you can focus on the product.",
...
function SpotlightFeatureCard({ card }: {
    card: SpotlightCard;
}) {
    const Icon = card.icon;
    return (<CardSpotlight color={card.color} radius={320} className="h-full p-8">
      <div className="flex h-full flex-col gap-5">
        ...
        <Link href={card.href} className="mt-auto inline-flex items-center gap-1.5 pt-2 text-sm font-medium text-primary transition-colors hover:text-foreground">
          {card.cta}
    ...
    return (<section className="px-6 py-20 sm:py-24">
      <div className="mx-auto w-full max-w-5xl">
        ...
        <div className="grid gap-6 md:grid-cols-2">
          {CARDS.map((card) => (<SpotlightFeatureCard key={card.title} card={card}/>))}
        </div>
      </div>
    </section>);
}
