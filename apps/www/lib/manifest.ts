...
export const manifest: ComponentMeta[] = [
  ...
  { slug: "heatmap", name: "Heatmap", description: "热力图 · 网格色阶映射(value→bucket→primary 透明度档·纯函数 buildMatrix/bucketize 可测) + 行列标签 + 原生 hover 提示 + 点击下钻 · 代码热点/贡献活动/覆盖率·库内首个热力图", category: "data-display", group: "collection", status: "new" },
  { slug: "code-review-thread", name: "CodeReviewThread", description: "代码审查评论线程 · severity 左边色条(四级语气) + AI/人类作者 + 内嵌建议修改 diff 可采纳 + 回复/标记已解决·误报 + 折叠 · 嵌 code-diff annotations 槽或独立用·复用 Avatar/Tag/CodeDiff", category: "data-display", group: "collection", status: "new" },
