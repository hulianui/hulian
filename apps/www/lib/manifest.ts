...
export const manifest: ComponentMeta[] = [
  ...
  // —— react-bits 移植批 ——
  { slug: "ascii-text", name: "ASCIIText", description: "把文字渲染成 ASCII 字符画 · canvas2d 逐像素亮度采样映射字符 + 正弦波动位移 + 鼠标驱动 hue-rotate 色相（零依赖去 three.js·token·reduced-motion）", category: "typography", group: "text", tags: ["animated"], status: "new" },
  { slug: "curved-loop", name: "CurvedLoop", description: "沿二次贝塞尔曲线无缝循环滚动的弧形跑马灯文字 · SVG textPath + rAF 续滚 + 可拖拽拨动(零依赖·token/currentColor·reduced-motion)", category: "typography", group: "text", tags: ["animated"], status: "new" },
  { slug: "falling-text", name: "FallingText", description: "文字按词散落物理特效 · 触发后逐词受重力下落/撞墙反弹堆叠 + 指针拖拽抛掷(零依赖 RAF 自写二维刚体·token·reduced-motion)", category: "typography", group: "text", tags: ["animated"], status: "new" },
  { slug: "fuzzy-text", name: "FuzzyText", description: "信号噪点模糊标题 · canvas2d 逐行/逐列随机错位扫描 + 悬停增强抖动(零依赖·token·reduced-motion 退静态帧)", category: "typography", group: "text", tags: ["animated"], status: "new" },
  { slug: "scrambled-text", name: "ScrambledText", description: "指针靠近文字逐字翻滚乱码再收敛的悬停文字特效 · 半径内越近翻滚越久 + 可自定字符集/速度（零依赖 RAF·reduced-motion）", category: "typography", group: "text", tags: ["animated"], status: "new" },
  { slug: "reveal", name: "Reveal", description: "通用「逐级揭示」块级动效原语 · Reveal 包裹元素进入视口时从位移/透明/缩放过渡到正常 + Stagger/StaggerItem 让子项按序错峰入场（非文字动画 · 作用于任意块级内容 · motion 运行时 · 吃 token · reduced-motion 直出终态）", category: "decoration", group: "overlay-fx", tags: ["animated"], status: "new" },
  ...
  { slug: "scroll-velocity", name: "ScrollVelocity", description: "滚动速度跑马灯文字 · 多行交替方向视差 + 随页面滚动加速/变向、静止匀速漂移(motion 速度值驱动·零额外依赖·reduced-motion 冻结自走帧保留 DOM)", category: "typography", group: "text", tags: ["animated"], status: "new" },
  { slug: "shuffle", name: "Shuffle", description: "逐字「洗牌解密」文本动画 · 字位先滚乱码再按方向顺序锁定真字 + 视口/悬停/循环触发(零依赖 rAF·reduced-motion 直落终态·token 着色)", category: "typography", group: "text", tags: ["animated"], status: "new" },
  { slug: "text-cursor", name: "TextCursor", description: "光标拖尾文字特效组件 · 在容器内移动光标时沿轨迹按间距落下一串字形并随机微浮动淡出回收（沿方向旋转 / emoji / 自定义间距上限）(零依赖·motion 运行时·reduced-motion 去浮动)", category: "typography", group: "text", tags: ["animated"], status: "new" },
  ...
  { slug: "variable-proximity", name: "VariableProximity", description: "鼠标邻近驱动的逐字可变字体文本 · 按距离插值 font-variation-settings（wght/opsz 等轴）+ linear/exponential/gaussian 三档衰减（零依赖 RAF · reduced-motion 定格 from · sr-only 完整朗读）", category: "typography", group: "text", tags: ["animated"], status: "new" },
  { slug: "antigravity", name: "Antigravity", description: "反重力粒子吸附背景 · 光标靠近把漂浮粒子吸入环绕轨道(波动+脉冲+取向)，离开缓回原位 · 磁吸半径/环半径/形状/自动巡游可调(canvas2d 零依赖·token 配色·reduced-motion 静态点阵)", category: "decoration", group: "overlay-fx", tags: ["animated"], status: "new" },
  { slug: "blob-cursor", name: "BlobCursor", description: "果冻光标拖尾组件 · 领头跟手 + 多滴弹性拖尾经 SVG gooey 滤镜融成液态水银(去 gsap·motion useSpring·token·reduced-motion)", category: "decoration", group: "overlay-fx", tags: ["animated"], status: "new" },
  { slug: "click-spark", name: "ClickSpark", description: "点击迸发火花的交互背景包裹器 · 点击点放射一圈短线段沿角度缓动飞散 + 数量/半径/长度/时长/缓动全可调（canvas2d+RAF 零依赖·token 取色·reduced-motion 静默·RSC 安全·jsdom 安全）", category: "decoration", group: "overlay-fx", tags: ["animated"], status: "new" },
  { slug: "crosshair", name: "Crosshair", description: "鼠标跟随的容器内准星十字线 · lerp 平滑拖尾 + 进入抖动脉冲 + token 配色(零依赖·reduced-motion)", category: "decoration", group: "overlay-fx", tags: ["animated"], status: "new" },
  { slug: "electric-border", name: "ElectricBorder", description: "通电跳动的边框装饰 · SVG 湍流位移描边 + 多层模糊光晕模拟放电辉光(零依赖·token·reduced-motion)", category: "decoration", group: "overlay-fx", tags: ["animated"], status: "new" },
  { slug: "ghost-cursor", name: "GhostCursor", description: "跟随指针的幽灵烟雾拖尾 · fbm 噪声烟团 + 环形帧缓冲历史尾迹 + 惯性滑行渐隐（ogl WebGL·去 three.js·reduced-motion 降级）", category: "decoration", group: "overlay-fx", tags: ["animated"], status: "new" },
  { slug: "gradual-blur", name: "GradualBlur", description: "沿容器某一边叠加多层 backdrop-filter 的渐进式模糊贴边 · 八方向/曲线/指数递增 + 悬停增强 + 进场淡入(零依赖·tokens·reduced-motion)", category: "decoration", group: "overlay-fx", tags: ["animated"], status: "new" },
  { slug: "image-trail", name: "ImageTrail", description: "光标拖尾图片组件 · 移动光标沿轨迹依次甩出图片淡入跟随、缩小淡出(零依赖 RAF+lerp·token·reduced-motion)", category: "decoration", group: "overlay-fx", tags: ["animated"], status: "new" },
  { slug: "laser-flow", name: "LaserFlow", description: "自顶向下倾泻的体积激光束 WebGL 背景 · 极坐标光束几何 + fbm 噪声雾 + 行进微流光 + 指针牵引倾斜(ogl 去 three·主题感知·reduced-motion 降级)", category: "decoration", group: "overlay-fx", tags: ["animated"], status: "new" },
  { slug: "magic-rings", name: "MagicRings", description: "同心魔法光环装饰背景 · GLSL 循环扩张/淡入淡出双色波纹 + 噪点颗粒 + 鼠标视差/悬停缩放/点击爆发（ogl·token 双色·reduced-motion 静态同心环降级）", category: "decoration", group: "overlay-fx", tags: ["animated"], status: "new" },
  { slug: "magnet", name: "Magnet", description: "指针磁吸交互组件 · 内容按「到中心距离/强度」牵引跟随指针、离开平滑归位(零依赖·reduced-motion 钳位)", category: "decoration", group: "overlay-fx", tags: ["animated"], status: "new" },
  { slug: "magnet-lines", name: "MagnetLines", description: "指针驱动的磁力线网格 · rows×columns 细线段实时朝向鼠标(acos 反三角求角) + token 线色(零依赖·reduced-motion 静止)", category: "decoration", group: "overlay-fx", tags: ["animated"], status: "new" },
  { slug: "meta-balls", name: "MetaBalls", description: "一组黏液球（metaball）公转游走、距离平方反比势场叠加 smoothstep 等值面产生融合/分裂的有机黏连背景 · 主体球与跟随鼠标的光标球交界处混色 + 自动巡游兜底(ogl·token 配色·reduced-motion 静态渐变球团 fallback)", category: "decoration", group: "overlay-fx", tags: ["animated"], status: "new" },
  ...
  { slug: "orbit-images", name: "OrbitImages", description: "沿轨道环绕流转的子项编排组件 · 9 种形状预设(椭圆/圆/方/三角/星/心/无穷/波浪+custom path)+等距铺满/鱼贯出发+可倾斜+中心叠层(纯 CSS offset-path·零依赖·RSC 安全·reduced-motion)", category: "decoration", group: "overlay-fx", tags: ["animated"], status: "new" },
  { slug: "pixel-trail", name: "PixelTrail", description: "鼠标划过点亮背后的像素网格、拖尾随时间淡灭的颗粒余晖背景 · 自维护 CPU 拖尾缓冲 + ogl data texture 着色 + 可选 gooey 液态融合(ogl·token·reduced-motion)", category: "decoration", group: "overlay-fx", tags: ["animated"], status: "new" },
  { slug: "pixel-transition", name: "PixelTransition", description: "像素马赛克转场卡片：双层内容叠放，悬停/聚焦/点击触发一幕随机散入再散出的像素幕布完成切换 · 自定网格密度/过场时长/只进不退 + token 像素色（去 gsap·motion 减包驱动·reduced-motion 直切）", category: "decoration", group: "overlay-fx", tags: ["animated"], status: "new" },
  { slug: "ribbons", name: "Ribbons", description: "弹簧追随鼠标的飘带 WebGL 效果 · ogl Polyline 多条折线弹性拖尾 + 可选沿带渐隐/正弦波动(ogl·token·reduced-motion)", category: "decoration", group: "overlay-fx", tags: ["animated"], status: "new" },
  { slug: "shape-blur", name: "ShapeBlur", description: "鼠标揭示的模糊形状高光 WebGL 件 · 圆角矩形/圆/圆环/三角四形态 + 阻尼跟随柔光圆擦亮边缘(ogl·token·reduced-motion 降级静态光晕)", category: "decoration", group: "overlay-fx", tags: ["animated"], status: "new" },
  { slug: "splash-cursor", name: "SplashCursor", description: "指针流体溅射光标特效 · 移动溅彩+点击爆斑+拖尾消散 · 彩虹色相轮/固定 chart token 双模(canvas2d 零依赖·reduced-motion·RSC 安全)", category: "decoration", group: "overlay-fx", tags: ["animated"], status: "new" },
  ...
  { slug: "target-cursor", name: "TargetCursor", description: "跟随鼠标的准星自定义光标 · 中心圆点 + 四角括号空闲自转、悬停命中元素时展开包裹其包围盒(去 gsap·裸 rAF lerp·token 着色·reduced-motion)", category: "decoration", group: "overlay-fx", tags: ["animated"], status: "new" },
  { slug: "cubes", name: "Cubes", description: "3D 立方体阵列交互背景 · 指针靠近按距离衰减倾斜 + 空闲自动游走 + 点击环形涟漪高亮（零依赖去 gsap·token 配色·reduced-motion）", category: "decoration", group: "overlay-fx", tags: ["animated"], status: "new" },
  { slug: "logo-loop", name: "LogoLoop", description: "无缝无限滚动的 logo 跑马灯 · RAF 速度低通平滑 + 序列复制无缝回卷 · 横纵四向/悬停变速暂停/单项放大/两端渐隐(零依赖·token·reduced-motion)", category: "data-display", group: "collection", tags: ["animated"], status: "new" },
  ...
  { slug: "staggered-menu", name: "StaggeredMenu", description: "侧滑分层导航菜单 · 触发按钮（菜单↔关闭文字滚动 + 加号旋转 225°）唤起面板 · 多层色层错峰滑入做叠纸质感 + 主条目自下而上带旋转依次入场 + 序号/社交链接尾随淡入（motion 去 gsap·token·reduced-motion）", category: "navigation", group: "global", tags: ["animated"], status: "new" },
  { slug: "bounce-cards", name: "BounceCards", description: "一叠卡片扇形铺开的入场弹跳组件 · 逐张 scale 0 弹性弹入 + hover 推挤让位（零依赖去 gsap·motion 弹簧·reduced-motion）", category: "data-display", group: "collection", tags: ["animated"], status: "new" },
  { slug: "card-swap", name: "CardSwap", description: "3D 透视卡片自动洗牌组件 · 最前卡下坠 → 其余递进 → 绕回队尾循环 + 复合 CardSwap.Card 默认皮肤（去 gsap·motion useAnimate·reduced-motion）", category: "data-display", group: "collection", tags: ["animated"], status: "new" },
  { slug: "chroma-grid", name: "ChromaGrid", description: "光标聚光揭示的卡片墙 · 整片灰度暗化、光标圈内透出全彩 + 单卡 hover 径向高光（弹性跟随·reduced-motion）", category: "data-display", group: "collection", tags: ["animated"], status: "new" },
  { slug: "circular-gallery", name: "CircularGallery", description: "ogl 弧形图片画廊 · 卡片沿圆弧弯曲倾斜排开 + 滚轮/拖拽惯性平移首尾无缝循环(ogl·圆角SDF·token化标题·reduced-motion 降级静态轨道)", category: "data-display", group: "collection", tags: ["animated"], status: "new" },
  ...
  { slug: "infinite-menu", name: "InfiniteMenu", description: "可拖拽旋转的菜单球，菜单项围绕球面分布、正对镜头者贴靠为激活项并显覆盖层标题/描述+动作箭头 · 松手惯性衰减 + 自动自旋（零依赖 CSS-3D/RAF·reduced-motion）", category: "data-display", group: "collection", tags: ["animated"], status: "new" },
  { slug: "magic-bento", name: "MagicBento", description: "魔法便当卡片网格 · 光标跟随径向聚光 + 描边光呼吸 + 可选 3D 倾斜(零依赖·token·reduced-motion)", category: "data-display", group: "collection", tags: ["animated"], status: "new" },
  { slug: "scroll-stack", name: "ScrollStack", description: "滚动驱动的卡片堆叠组件 · 逐张钉住 + 缩放递进 + 可选旋转/景深模糊(零依赖原生滚动+RAF·token 皮肤·reduced-motion 静态平铺)", category: "data-display", group: "collection", tags: ["animated"], status: "new" },
  { slug: "profile-card", name: "ProfileCard", description: "指针倾斜 + 全息炫彩名片卡 · RAF 阻尼弹簧驱动 3D 旋转/光晕（零依赖）+ chart token 全息渐变 + 无头像落首字母占位（reduced-motion 静态降级）", category: "data-display", group: "collection", tags: ["animated"], status: "new" },
  { slug: "tilted-card", name: "TiltedCard", description: "指针驱动的 3D 倾斜卡片 · perspective + rotateX/Y 弹簧倾斜 + 悬停放大 + 跟随指针的浮动提示气泡（motion 弹簧·零新依赖·reduced-motion）", category: "data-display", group: "collection", tags: ["animated"], status: "new" },
  { slug: "pixel-card", name: "PixelCard", description: "canvas2d 像素卡片 · 悬停/聚焦时像素自中心波纹生长+闪烁、离开收缩消散 · 4 变体/可定制 gap·speed·colors(零依赖 RAF·token 配色·reduced-motion 静止)", category: "data-display", group: "collection", tags: ["animated"], status: "new" },
  { slug: "decay-card", name: "DecayCard", description: "鼠标驱动的「湍流溶解」图片卡 · SVG feTurbulence+feDisplacementMap 随鼠标速度融化图像 + 带阻尼的视差倾斜平移（零依赖去 gsap·RAF 缓动·reduced-motion）", category: "data-display", group: "collection", tags: ["animated"], status: "new" },
  ...
  { slug: "folder", name: "Folder", description: "点击展开的 3D 文件夹 · 三层纸张扇形铺开 + 展开后磁吸跟随鼠标(零依赖纯 CSS transform·token 派生·reduced-motion)", category: "data-display", group: "collection", tags: ["animated"], status: "new" },
  { slug: "border-glow", name: "BorderGlow", description: "指针感应发光边框卡片 · 沿光锥点亮彩色网格边框 + 外层霓虹光晕(随边缘贴近度渐变) + 可选挂载自动扫光(零依赖·token·reduced-motion)", category: "decoration", group: "overlay-fx", tags: ["animated"], status: "new" },
  { slug: "glass-icons", name: "GlassIcons", description: "玻璃拟态图标按钮网格 · 背面 3D 旋转彩色发光板 + 前面磨砂玻璃片（backdrop-blur·inset 高光边），hover/focus 彩板抬升旋转、玻璃前推、标签滑出（纯 CSS 零依赖·token 配色·RSC 安全·reduced-motion）", category: "decoration", group: "overlay-fx", tags: ["animated"], status: "new" },
  ...
  { slug: "lanyard", name: "Lanyard", description: "可拖拽的挂绳工牌 · 单摆+阻尼弹簧物理回弹(原生 PointerEvents+RAF·零三方依赖) + SVG 二次贝塞尔挂绳随摆角实时弯折 + token 配色(挂绳 primary·工牌 surface/border)·reduced-motion 松手即静止归位", category: "decoration", group: "overlay-fx", tags: ["animated"], status: "new" },
  { slug: "model-viewer", name: "ModelViewer", description: "交互式 3D 模型查看舞台 · 拖拽旋转(惯性) + 鼠标视差 + 悬停倾斜 + 自动旋转 + 接触阴影(零依赖 CSS 3D · RAF · reduced-motion)", category: "decoration", group: "overlay-fx", tags: ["animated"], status: "new" },
  { slug: "fluid-glass", name: "FluidGlass", description: "跟随指针的流体玻璃折射背景 · 程序化流动渐变底图 + 圆形玻璃透镜实时折射/放大/边缘色散/菲涅尔高光(ogl 单 shader·零 3D 依赖·reduced-motion)", category: "decoration", group: "overlay-fx", tags: ["animated"], status: "new" },
  ...
  { slug: "ballpit", name: "Ballpit", description: "一坑彩色小球的可交互物理球池 · 重力下落+墙壁回弹+球球弹性碰撞+光标排斥(零依赖 canvas2d·chart token·reduced-motion)", category: "decoration", group: "backdrop", tags: ["animated", "webgl"], status: "new" },
  { slug: "beams", name: "Beams", description: "流动体积光柱 WebGL 背景 · perlin 噪声扰动起伏 + 方向光受光梯度 + 倾斜光幕 + 胶片颗粒(ogl·token·reduced-motion 降级静态渐变)", category: "decoration", group: "backdrop", tags: ["animated", "webgl"], status: "new" },
  ...
  { slug: "dark-veil", name: "DarkVeil", description: "CPPN 神经场暗色流动帷幕 WebGL 背景 · 色相旋转/扫描线/颗粒/空间扭曲(ogl·懒加载·reduced-motion 降级静态渐变)", category: "decoration", group: "backdrop", tags: ["animated", "webgl"], status: "new" },
  { slug: "dither", name: "Dither", description: "复古有序抖动波纹 WebGL 背景 · Perlin/fbm 波纹场 + 8×8 Bayer 抖动 + 色阶量化/像素马赛克(ogl 单遍 shader·token·reduced-motion 降级)", category: "decoration", group: "backdrop", tags: ["animated", "webgl"], status: "new" },
  ...
  { slug: "faulty-terminal", name: "FaultyTerminal", description: "出故障的 CRT 终端字符雨 WebGL 背景 · fbm 噪声点阵 + 横向撕裂/扫描线/闪烁/桶形畸变/色散 + 鼠标涟漪(ogl·token 着色·reduced-motion 兜底)", category: "decoration", group: "backdrop", tags: ["animated", "webgl"], status: "new" },
  { slug: "ferrofluid", name: "Ferrofluid", description: "液态金属铁磁流体 WebGL 背景组件 · value-noise 峰脊 + smin 软融合 + rim 亮带流动 + 鼠标下凹交互(ogl·主题 chart token·reduced-motion 径向渐变兜底)", category: "decoration", group: "backdrop", tags: ["animated", "webgl"], status: "new" },
  ...
  { slug: "galaxy", name: "Galaxy", description: "程序化生成的多层视差星河 WebGL 背景 · Hash 铺点+十字辉光+HSV 调色+三角波闪烁+自转/鼠标斥力，4 层尺度叠加深空纵深(ogl·懒加载·StrictMode 安全·reduced-motion 降级 chart token 径向渐变)", category: "decoration", group: "backdrop", tags: ["animated", "webgl"], status: "new" },
  ...
  { slug: "grainient", name: "Grainient", description: "三色域扭曲 + 胶片颗粒的 WebGL 渐变背景 · 噪声驱动旋转 + 实时 grain/对比度后期(ogl·reduced-motion 降级静态渐变)", category: "decoration", group: "backdrop", tags: ["animated", "webgl"], status: "new" },
  { slug: "grid-distortion", name: "GridDistortion", description: "鼠标拖拽的液态网格扭曲 WebGL 背景 · 数据纹理位移场 + 鼠标速度涟漪/弛豫回弹(ogl·零外部资源默认程序化网格底纹·token 着色·reduced-motion 降级)", category: "decoration", group: "backdrop", tags: ["animated", "webgl"], status: "new" },
  { slug: "grid-motion", name: "GridMotion", description: "倾斜网格视差背景 · 鼠标横移每行奇偶反向弹性追随 + 中心径向光晕(零依赖 motion useSpring·token·reduced-motion)", category: "decoration", group: "backdrop", tags: ["animated", "webgl"], status: "new" },
  { slug: "grid-scan", name: "GridScan", description: "透视扫描网格 WebGL 背景 · 射线投射无限网格(实线/虚线/点线)+纵深推进的发光扫描脉冲+鼠标视差偏摆(ogl·token·reduced-motion 降级静态网格)", category: "decoration", group: "backdrop", tags: ["animated", "webgl"], status: "new" },
  { slug: "hyperspeed", name: "Hyperspeed", description: "超空间跃迁 warp 隧道背景 · 从消失点放射的双色车灯光带向观察者冲来 + 湍流扭曲 + 程序化辉光(ogl 单 shader·零新依赖·token·reduced-motion 静态降级)", category: "decoration", group: "backdrop", tags: ["animated", "webgl"], status: "new" },
  { slug: "letter-glitch", name: "LetterGlitch", description: "终端字符故障雨背景 · canvas 等宽字符矩阵按节拍随机翻字翻色 + 颜色逐帧插值平滑 + 内/外缘暗角(零依赖 canvas2d·token 调色板·reduced-motion)", category: "decoration", group: "backdrop", tags: ["animated", "webgl"], status: "new" },
  { slug: "lightfall", name: "Lightfall", description: "隧道光束坠落 WebGL 背景 · 多色光束按高度循环取色 + 中心辉光 + 闪烁/拖尾 + 鼠标牵引(ogl·token·reduced-motion 降级)", category: "decoration", group: "backdrop", tags: ["animated", "webgl"], status: "new" },
  { slug: "lightning", name: "Lightning", description: "fbm 噪声电弧/极光柱 WebGL 背景 · 距离场反比辉光 + hash 随机闪烁(ogl·token 取色·reduced-motion 静态降级)", category: "decoration", group: "backdrop", tags: ["animated", "webgl"], status: "new" },
  ...
  { slug: "light-rays", name: "LightRays", description: "体积光束放射 WebGL 背景 · 八向原点 + 双层正弦扰动/扩散衰减/脉动/噪点/扭曲 + 鼠标方向跟随(ogl·token 着色·reduced-motion 静态渐变兜底)", category: "decoration", group: "backdrop", tags: ["animated", "webgl"], status: "new" },
  { slug: "line-waves", name: "LineWaves", description: "流动波纹线阵 WebGL 背景 · 双正弦位移场 + 噪声脊线 + 三通道色相循环 + 鼠标局部隆起(ogl·chart token 配色·reduced-motion 静态线纹降级)", category: "decoration", group: "backdrop", tags: ["animated", "webgl"], status: "new" },
  { slug: "liquid-ether", name: "LiquidEther", description: "鼠标驱动的液态色域 WebGL 背景 · 域扭曲+metaball 翻涌 + 指针搅动/自动巡游(ogl·token·reduced-motion 降级)", category: "decoration", group: "backdrop", tags: ["animated", "webgl"], status: "new" },
  { slug: "pixel-blast", name: "PixelBlast", description: "抖动点阵翻涌的 WebGL 背景 · FBM 噪声 + 8×8 Bayer 有序抖动量化成方块/圆/三角/菱形像素 + 四周渐隐（ogl·token·reduced-motion 降级静态点阵）", category: "decoration", group: "backdrop", tags: ["animated", "webgl"], status: "new" },
  { slug: "pixel-snow", name: "PixelSnow", description: "像素化体素雪场 WebGL 背景 · 光线步进逐格哈希生雪 + 景深淡出 · 方块/圆点/六臂雪花三变体（ogl 去 three.js·token 着色·reduced-motion 静态点阵降级）", category: "decoration", group: "backdrop", tags: ["animated", "webgl"], status: "new" },
  ...
  { slug: "shape-grid", name: "ShapeGrid", description: "无限滚动几何网格背景 · canvas2d 绘制方/圆/三角/六边形四形，沿任意方向匀速平移环绕 + 鼠标悬停缓动填充与渐隐拖尾（零依赖·token 配色·reduced-motion 静止·jsdom 安全）", category: "decoration", group: "backdrop", tags: ["animated", "webgl"], status: "new" },
  { slug: "side-rays", name: "SideRays", description: "从屏幕一角发散的动态侧光束 WebGL 背景 · 双束角度正弦摆动+平方反比衰减+饱和度/混色调节(ogl·token 双色·reduced-motion 静态兜底)", category: "decoration", group: "backdrop", tags: ["animated", "webgl"], status: "new" },
  { slug: "soft-aurora", name: "SoftAurora", description: "WebGL 柔和极光背景 · 3D Perlin 噪声 + cosine 渐变双层叠加 · 鼠标视差(ogl·token 着色·reduced-motion 降级)", category: "decoration", group: "backdrop", tags: ["animated", "webgl"], status: "new" },
];
