...
export const manifest: ComponentMeta[] = [
  ...
  { slug: "preview-sandbox", name: "PreviewSandbox", description: "预览沙箱 · iframe 隔离渲染(code=完整 HTML 文档串→srcDoc·默认 allow-scripts 不透明源·错误经注入脚本 postMessage 回传)与同文档 React 错误边界双模式共用一副壳(错误对象同形状) + 设备视口/自由尺寸(切设备不重挂 iframe 不重载文档) + 复用 IPhone/Android/Tablet 外框 + fit 缩放刻意不放大 + 错误态与重试 · 尺寸表/缩放系数/错误归一化皆为带单测的纯函数 · 不做代码执行引擎(不打包不转译)", category: "layout", group: "container", status: "new" },
  ...
  { slug: "flip-text", name: "FlipText", description: "翻面标题 · 鼠标移入时逐字 3D 翻面(收 children 不收 text · as 参与类型推导让它自己就是 h1/h2 · 一次性播完不跟随 hover 回退 · 纯 CSS 关键帧不引 motion · 背面走伪元素不污染 textContent)", category: "typography", group: "text", tags: ["animated"], status: "new" },
  { slug: "text-reveal", name: "TextReveal", description: "揭示扫光 · 一条多色带横扫把文字从透明揭示成实色(repeat 循环表示「进行中」· 多串轮换按最宽串预留宽度不跳 · 减弱动效落静态终态而非整串消失 · 纯 CSS 关键帧不引 motion)", category: "typography", group: "text", tags: ["animated"], status: "new" },
  ...
  { slug: "cell-editor", name: "CellEditor", description: "单元格内联编辑器 · 逐格常驻编辑原语(静止态与纯文本同形/失焦或 Enter 即提交/值没变不发/Esc 回滚/missing 灰斜体/field-sizing 自增高/onCommit 返 Promise 自带 pending)", category: "forms", group: "basic", status: "new" },
  ...
  { slug: "rich-text-editor", name: "RichTextEditor", description: "富文本编辑器 · 值进出 HTML 字符串 + 工具栏可裁 + 图片上传注入 + 粘贴净化", category: "forms", group: "advanced", status: "new" },
  { slug: "code-editor", name: "CodeEditor", description: "代码编辑器 · 零依赖 textarea+高亮层叠加(不引 CodeMirror/Monaco) · 受控 value/onChange + 行号槽 + 当前行高亮 + 语法着色(JS 家族/JSON/Shell 复用 CodeBlock 着色器·CSS 自带带状态扫描器) + 键盘全套(Tab 缩进/Shift+Tab 反缩进/Enter 续缩进/成对符号自动闭合与包裹/退格删对/Cmd 或 Ctrl 加斜杠切注释)且一律经 execCommand 落笔保住原生 undo 栈 · 编辑意图抽成纯函数带单测 · 不做折叠/补全/多光标/minimap", category: "forms", group: "advanced", status: "new" },
  { slug: "issue-reporter", name: "IssueReporter", description: "GitHub issue 草稿器 · 表单收集 → 模板纯函数拼 Markdown → 回吐结构化草稿 + 生成预填链接 · 链接超长自动降级为复制 Markdown(判据量整条 URL 不只 body) · 内置 bug/feature/enhancement 三套模板且可换 · 附 IssueReporterModal 弹层版 · 不调 API 不持 token", category: "forms", group: "advanced", status: "new" },
  ...
  { slug: "inspector-panel", name: "InspectorPanel", description: "属性检查器 · 设计工具的字段 schema 驱动属性面板，按 kind 派生控件 · spacing 四联链接锁定 / 主题 token 色板 / MIXED 混合值占位 / commitMode 控制回吐时机 · 内置 layout/color/typography/border/effects 五套预设 schema", category: "forms", group: "advanced", status: "new" },
  ...
  { slug: "design-canvas", name: "DesignCanvas", description: "视觉设计画布 · 无限平移缩放 + 元素选择框 + 拖拽移动与八向 resize · 受控 items 托管几何、children 作自绘图层 · 键盘可达、几何抽纯函数带单测 · 区别 Flow(节点编排)：这是自由排列的设计画布", category: "data-display", group: "collection", status: "new" },
  { slug: "component-picker", name: "ComponentPicker", description: "组件库浏览器 · 左分类树(复用 Tree) + 模糊搜索 + 结果网格(Card) + 详情面板(预览/Props 表/示例代码三开关) · 自研零依赖打分器(slug 命中远重于描述命中·不引 fuse.js) + parseComponentCatalog 把 llms-full.txt 解析成条目(纯函数·在消费方那层跑·实测 376/376 slug 全对) + renderPreview 注入 live 预览(库不 eval 不 iframe) · 附 ComponentPickerCommand 薄封装走 ⌘K", category: "data-display", group: "collection", status: "new" },
  ...
  { slug: "treemap", name: "Treemap", description: "矩形树图 · 一组扁平数据按 value 占面积铺满矩形，「谁占大头」一眼可见 · recharts squarify 引擎 + chart token 皮肤 · 格内文字按格子尺寸自动取舍(放不下就不画·treemapLabelFit 纯函数带单测) + onItemClick 钻取 + valueFormat 统管格内与 tooltip · 单层不下钻", category: "data-display", group: "collection", status: "new" },
  ...
  { slug: "sidebar", name: "Sidebar", description: "应用侧栏 · 组合式外壳 + 折叠状态机(offcanvas/icon/none·受控/非受控两用·Cmd/Ctrl+B 快捷键在输入态自动让路) + 移动端自动切抽屉(复用 Drawer·sr-only 标题与说明) + 22 个结构槽位(Header/Content/Footer/Group/Menu/MenuButton 带 render 注入与 icon 档 Tooltip/MenuAction 兄弟节点不造 button 套 button/MenuSkeleton 确定宽度) · in-flow 宽度过渡不用 fixed，可嵌任意容器", category: "navigation", group: "global", status: "new" },
  ...
  { slug: "element-selection-overlay", name: "ElementSelectionOverlay", description: "元素选择叠加层 · 在容器或同源 iframe 里 hover 高亮/点击选中元素并回吐组件树路径 · 不往目标文档写任何样式 · marked(data-hulian-path) 与 structural 两层路径且对外暴露可靠度 · 跨源 iframe 明确报错不装接上了 · 指向编辑基础设施", category: "feedback", group: "overlay", status: "new" },
