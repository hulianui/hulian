...
export function ThemeSidebar() {
  const shell = useIntlayer("site-shell");
  const pathname = usePathname();
  const router = useRouter();
  const activeKey = keyOf(THEME_NAV.find((i) => hrefOf(i.slug) === stripDocsBasePath(pathname))?.slug ?? "");
  ...
  return (
    <NavMenu
      aria-label={shell.themeNavigation}
      className="!w-full"
      items={items}
      selectedKeys={[activeKey]}
      onSelect={(key) => {
        const item = THEME_NAV.find((i) => keyOf(i.slug) === key);
        if (item) router.push(stripDocsBasePath(withDocsBasePath(hrefOf(item.slug))));
      }}
    />
  );
}
