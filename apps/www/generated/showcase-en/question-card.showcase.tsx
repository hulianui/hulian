...
import type { QuestionType } from "../../../../packages/ui/src/question/question.types";
...
/>`,
          ...
          <QuestionCard number="3" type="single" difficulty="A Group" stem="As shown in the picture, figures ① and ② are made up of exactly the same small squares. If the side length of graph ① is 4, then the area of ​​graph ② is expressed as a fraction ( )." options={[
                    { key: "A", text: "\\frac{1}{9}" },
                    { key: "B", text: "\\frac{5}{9}" },
                    { key: "C", text: "\\frac{16}{9}" },
                    { key: "D", text: "\\frac{80}{9}" },
...
/>`,
            render: () => (<div className="w-full max-w-2xl">
          <QuestionCard number="11" type="blank" stem="Profit is stipulated to be positive. A company lost 30,000 yuan last year, which can be recorded as ____ million yuan." parts={["(1) The number b represented by the point B on the number axis is ____.", "(2) The number represented by point P is ____."]} chapter="Chapter 1 Rational Numbers"/>
        </div>),
        },
        {
...
/>`,
            render: () => (<div className="w-full max-w-2xl">
          <QuestionCard number="7" type="single" stem="Among the following formulas, the correct one is ( )." options={[{ key: "A", text: "-|-16|>0" }]} issues={[{ label: "Less than 4 options" }, { label: "Question numbers are not consecutive" }]} actions={<Button size="sm" variant="ghost">
                Go to proofreading
              </Button>}/>
        </div>),
        },
    ...
    states: [
        {
            ...
            render: () => (<QuestionCard number="2" type="single" stem="Convert \\frac{3}{8} into a decimal ( )." options={[
                    { key: "A", text: "0.125" },
                    { key: "B", text: "0.250" },
                    { key: "C", text: "0.375" },
                    { key: "D", text: "0.625" },
        ...
        },
        {
            ...
            render: () => <QuestionCard number="11" type="blank" stem="A loss of 30,000 yuan last year can be recorded as ____ million yuan."/>,
        },
        {
            name: "Answer the question",
            render: () => (<QuestionCard number="17" type="essay" stem="Fill in the following numbers into the corresponding collection circles:" parts={["(1) Positive number set", "(2) Negative integer set"]}/>),
        },
        {
            name: "Pending review",
            render: () => (<QuestionCard number="7" type="single" stem="Among the following formulas, the correct one is ( )." issues={[{ label: "Less than 4 options" }]}/>),
        },
        {
            name: "Compact",
            render: () => <QuestionCard number="5" type="blank" stem="When m<0, |-3m|=____." compact/>,
        },
    ],
    renderWithProps: (p) => (<QuestionCard number={String(p.number ?? "")} type={(p.type as QuestionType) ?? "single"} difficulty={String(p.difficulty ?? "")} stem={String(p.stem ?? "")} compact={Boolean(p.compact)}/>),
    toCode: (p) => `<QuestionCard number="${String(p.number ?? "")}" type="${String(p.type ?? "single")}" stem={${JSON.stringify(String(p.stem ?? ""))}} />`,
};
