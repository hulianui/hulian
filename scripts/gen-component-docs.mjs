...
// every component file sits one level under src/<slug>/, so links to any sibling
// are uniformly `../<slug>/<slug>.md`.
const relLink = (slug) => `../${slug}/${slug}.md`;
...
function render(entry, f, entries) {
  ...
  const importLine = ex.length
    ? `import { ${ex.join(", ")} } from "@hulianui/ui"`
    : `import { /* ? */ } from "@hulianui/ui"`;
